...
export default function Page() {
  ...
  const generateConfigForMapping = (mapping: DomainMapping): string => {
    ...
    addLine("隐藏后端返回的 X-Frame-Options 头", "    proxy_hide_header X-Frame-Options;");
    addLine("隐藏后端返回的 X-Content-Type-Options 头", "    proxy_hide_header X-Content-Type-Options;");
    addLine("隐藏后端返回的 Referrer-Policy 头", "    proxy_hide_header Referrer-Policy;");
    addLine("隐藏后端返回的 Permissions-Policy 头", "    proxy_hide_header Permissions-Policy;");
    addLine("隐藏后端返回的 Strict-Transport-Security 头", "    proxy_hide_header Strict-Transport-Security;");
    addLine("隐藏后端返回的 Cross-Origin-Embedder-Policy 头", "    proxy_hide_header Cross-Origin-Embedder-Policy;");
    addLine("隐藏后端返回的 Cross-Origin-Opener-Policy 头", "    proxy_hide_header Cross-Origin-Opener-Policy;");
    addLine("隐藏后端返回的 Cross-Origin-Resource-Policy 头", "    proxy_hide_header Cross-Origin-Resource-Policy;");
    addLine("隐藏中间代理链路信息头 Via", "    proxy_hide_header Via;");
    addLine("隐藏后端技术栈头 X-Powered-By", "    proxy_hide_header X-Powered-By;");
    addLine("隐藏 Cloudflare 提供的 CF-RAY 头", "    proxy_hide_header CF-RAY;");
    addLine("隐藏 Cloudflare 提供的 CF-Cache-Status 头", "    proxy_hide_header CF-Cache-Status;");
          ...
          {mappings.length === 0 ? (
            ...
            <ScrollArea className="h-full">
              <div className="space-y-1 p-2">
                {mappings.map((mapping, index) => (
                  <div
                    key={`${mapping.source}-${mapping.proxy}`}
                    onClick={() => setSelectedIndex(index)}
                    className={`group relative cursor-pointer rounded-md border p-2 transition-colors hover:bg-accent ${
                      selectedIndex === index ? "border-primary bg-accent" : "border-border"
                    }`}
